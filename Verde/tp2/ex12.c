...
int comparaData(Date atual, Date prox) {
    
    int resp = 0;                                               

    if(resp == 0 && atual.year != prox.year) {                        
        resp = (atual.year< prox.year) ? -1 : 1;                //Realiza-se a comparação entre os atributos de data, de forma a retornar um "Integer"
    }                                                           //para que se posso posteriormente realizar a comparação de forma mais prática por valores.
...
int comparaShows(Show a, Show b) {
    ...
    char *tituloA = ToLower(a.title);
    char *tituloB = ToLower(b.title);

    resp = comparaData(a.date_added,b.date_added);            

    if(resp == 0) {
        comparacoes++;
        resp = strcmp(tituloA,tituloB);
    }
...
void ordenaPorBolha(Show *show, int tam) {
    for(int i=0;i<tam - 1;i++) {
        for(int j = 0;j<tam- i - 1;j++) {
            if(comparaShows(show[j],show[j+1])> 0) {
                movimentacoes++;
                Show temp = show[j];
                show[j] = show[j+1];
                show[j + 1] = temp;
