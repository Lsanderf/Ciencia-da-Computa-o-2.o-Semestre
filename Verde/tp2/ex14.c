...
int comparaShow(Show a, Show b) {
	int resp = 0;
	
	char *title1 = ToLower(a.title);
	char *title2 = ToLower(b.title);

	resp = strcmp(title1,title2);
...
int getMaior(Show *shows,int tam) {
	int maximo = shows[0].release_year;
    for(int i = 1; i<tam;i++) {
        if(shows[i].release_year > maximo) {
            maximo = shows[i].release_year;
        }
    }
...
void coutingSort(Show *shows, int tam, int exp) {
    Show *resultado = (Show *)calloc(tam,sizeof(Show));
    int contador[10] = {0};
    int i;

    for(i = 0;i<tam;i++) {
        contador[(shows[i].release_year / exp) % 10]++;
    }
    ...
    for(i = tam - 1; i >= 0; i--) {
        ...
        resultado[contador[digito] - 1] = shows[i];
        contador[digito]--;
        movimentacoes++;
    }
...
void ordenaTitulo(Show *shows, int tam) {
    int i = 0;

    while (i<tam) {

        int j = i;
        while(j<tam && shows[i].release_year == shows[j].release_year) {
            j++;
            comparacoes++;
        }
        ...
        for(int k = i; k < j - 1;k++) {
            for(int l = k + 1; l < j;l++) {
                if(comparaShow(shows[k],shows[l]) > 0 ) {
                    Swap(shows,k,l);
                }
...
void radixSort(Show *shows, int tam) {
    int max = getMaior(shows,tam);

    for(int exp = 1; max/exp > 0;exp *= 10) {
        coutingSort(shows,tam,exp);
    }

    ordenaTitulo(shows,tam);
}
