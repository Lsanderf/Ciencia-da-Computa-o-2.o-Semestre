...
int comparaShows(Show a, Show b) {
    int resp;
    char *tit1 = ToLower(a.title);
    char *tit2 = ToLower(b.title);
    char *dir1 = ToLower(a.director);
    char *dir2 = ToLower(b.director);

    comparacoes++; // Sempre conta uma comparação

    int compDir = strcmp(dir1, dir2);
    if (compDir != 0) {
        resp = compDir;
    } else {
        resp = strcmp(tit1, tit2);
    }
...
void heap(Show *shows, int tam,int i) {
    int raiz = i;
    int esq = (2 * i) + 1;
    int dir = (2 * i) + 2;

    if(esq < tam && comparaShows(shows[esq],shows[raiz]) > 0) {
        raiz  = esq;
    }

    if(dir < tam && comparaShows(shows[dir],shows[raiz]) > 0) {
        raiz = dir;
    }

    if(raiz != i) {
        Swap(&shows[raiz],&shows[i]);
        heap(shows,tam,raiz);
        
    }
 }

 void heapParcial(Show *shows,int tam, int k ) {
